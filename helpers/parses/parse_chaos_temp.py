...
#GET API KEY CHAOS
from apiKeys import getCHAOSKEY
...
def executa():
    subprocess.check_output('docker run --rm --name '+container_name+' -v $(pwd)/targets/'+target+'/temp:/data c4mund0n60-tools:1.0 chaos -duc -key ' + getCHAOSKEY() + ' -d '+domain+' -silent >> targets/'+target+'/temp/'+saida+' || true', shell=True)
    return 1
